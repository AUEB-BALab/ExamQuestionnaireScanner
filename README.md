...
TODO: create a subsection "Configuring ImageMagick" and place there the following guidelines
Note: 1. You should modify the polixy.xml and add the read & write permissions to the coder domain. In particular, in the ```/etc/ImageMagick-6/policy.xml``` or ```/etc/ImageMagick/policy.xml``` file change the line ```<policy domain="coder" rights="none" pattern="PDF" />``` to ```<policy domain="coder" rights="read|write" pattern="PDF" />```. 2. You might also need to modify the ```<policy domain="resource" name="memory" value="256MiB"/>``` and ```<policy domain="resource" name="disk" value="1GiB"/>``` and increase the values as much as possible.
